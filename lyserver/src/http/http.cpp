...
#undef XX
            ...
            #define PARSE_PARAM(str, m, flag, trim)                                                             \
                ...
                {                                                                                               \
                    ...
                    m.insert(std::make_pair(trim(str.substr(last, key - last)),                                 \
                                            lyserver::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)))); \
                    if (pos == std::string::npos)                                                               \
                    {                                                                                           \
                        break;                                                                                  \
                    }                                                                                           \
        ...
        void HttpRequest::initBodyParam()
        {
             ...
             * @brief (1) 成功找到，返回在"父串"中第一次出现的位置的 char *指针
        ...
        void HttpRequest::initCookies()
        {
            ...
            PARSE_PARAM(cookie, m_cookies, ';', lyserver::StringUtil::Trim);
            m_parserParamFlag |= 0x4;
        }
                                     ...
                                     const std::string &domain, bool secure)
        {
            ...
            if (expired > 0)
            {
                ss << ";expires=" << lyserver::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
            }
            if (!domain.empty())
            {
                ss << ";domain=" << domain;
