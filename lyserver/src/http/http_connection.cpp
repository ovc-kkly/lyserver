...
#include "log.h"
// #include "lyserver/streams/zlib_stream.h"
...
namespace lyserver
{
    namespace http
    {
        ...
        {
            LY_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
        }
        ...
        HttpResponse::ptr HttpConnection::recvResponse()
        {
            ...
            if (client_parser.chunked)
            {
                ...
                do
                {
                    ...
                    LY_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
            ...
            if (!body.empty())
            {
                ...
                LY_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
                                       << " size=" << body.size();
        ...
        HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string &uri, const std::string &vhost, uint32_t max_size, uint32_t max_alive_time, uint32_t max_request)
        {
            ...
            if (!turi)
            {
                LY_LOG_ERROR(g_logger) << "invalid uri=" << uri;
            }
        ...
        HttpConnectionPool::HttpConnectionPool(const std::string &host, const std::string &vhost, uint32_t port, bool is_https, uint32_t max_size, uint32_t max_alive_time, uint32_t max_request)
            : m_host(host), m_vhost(vhost), m_port(port ? port : (is_https ? 443 : 80)), m_maxSize(max_size), m_maxAliveTime(max_alive_time), m_maxRequest(max_request), m_isHttps(is_https)
        {
        }

        HttpConnection::ptr HttpConnectionPool::getConnection()
        {
            uint64_t now_ms = lyserver::GetCurrentMS();
            ...
            if (!ptr)//进入这里代表，连接池没连接，那么就创建一个连接对象
            {
                ...
                if (!addr)
                {
                    LY_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
                    return nullptr;
                }
                addr->setPort(m_port);
                ...
                if (!sock)
                {
                    LY_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
                    return nullptr;
                }
                if (!sock->connect(addr))
                {
                    LY_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
                    return nullptr;
                }
        ...
        {
            ++ptr->m_request;
            if (!ptr->isConnected() || ((ptr->m_createTime + pool->m_maxAliveTime) >= lyserver::GetCurrentMS()) || (ptr->m_request >= pool->m_maxRequest))
            {
                delete ptr;
                --pool->m_total;
                return;
