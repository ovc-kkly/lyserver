...
namespace lyserver
{
    ...
    Socket::ptr Socket::CreateTCP(lyserver::Address::ptr address)
    {
        Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
        return sock;
    }

    Socket::ptr Socket::CreateUDP(lyserver::Address::ptr address)
    {
    ...
    bool Socket::getOption(int level, int option, void *result, socklen_t *len)
    {
        ...
        if (rt)
        {
            LY_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
    ...
    bool Socket::setOption(int level, int option, const void *result, socklen_t len)
    {
        ...
        {
            LY_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
                                   << " level=" << level << " option=" << option
                                   << " errno=" << errno << " errstr=" << strerror(errno);
            return false;
        }
    ...
    Socket::ptr Socket::accept()
    {
        ...
        if (newsock == -1)
        {
            ...
            LY_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
    ...
    bool Socket::bind(const Address::ptr addr)
    {
        ...
        {
            newSock();
            if (LY_UNLIKELY(!isValid()))
            {
                return false;
            }
        }

        if (LY_UNLIKELY(addr->getFamily() != m_family))
        {
            LY_LOG_ERROR(g_logger) << "bind sock.family("
        ...
        if (uaddr)
        {
            ...
            {
                lyserver::FSUtil::Unlink(uaddr->getPath(), true);
            }
        }

        if (::bind(m_sock, addr->getAddr(), addr->getAddrLen()))
        {
            LY_LOG_ERROR(g_logger) << "bind error errrno=" << errno
    ...
    bool Socket::reconnect(uint64_t timeout_ms)
    {
        if (!m_remoteAddress)
        {
            LY_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
            return false;
        }
    ...
    bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms)
    {
        ...
        if (LY_UNLIKELY(addr->getFamily() != m_family))
        {
            LY_LOG_ERROR(g_logger) << "connect sock.family("
                                   << m_family << ") addr.family(" << addr->getFamily()
                                   << ") not equal, addr=" << addr->toString();
            return false;
        }
    ...
    bool Socket::listen(int backlog)
    {
        if (!isValid())
        {
            LY_LOG_ERROR(g_logger) << "listen error sock=-1";
            return false;
        }
        if (::listen(m_sock, backlog))
        {
            LY_LOG_ERROR(g_logger) << "listen error errno=" << errno
                                   << " errstr=" << strerror(errno);
            return false;
        }
        return true;
    ...
    Address::ptr Socket::getLocalAddress()
    {
        ...
        {
            LY_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
                                   << " errno=" << errno << " errstr=" << strerror(errno);
            return Address::ptr(new UnknownAddress(m_family));
        }
        if (m_family == AF_UNIX)
    ...
    void Socket::initSock()
    {
        ...
        {
            setOption(IPPROTO_TCP, TCP_NODELAY, val);//设置无延迟发生接收
        }
    }
    ...
    void Socket::newSock()
    {
        ...
        if (LY_LIKELY(m_sock != -1))
        {
            initSock();
        }
        else
        {
            LY_LOG_ERROR(g_logger) << "socket(" << m_family
                                   << ", " << m_type << ", " << m_protocol << ") errno="
                                   << errno << " errstr=" << strerror(errno);
        }
    }
    int Socket::sockfcntl(int fd, int __cmd, ...)
    {
        va_list args;
        va_start(args, __cmd);
    ...
    bool SSLSocket::loadCertificates(const std::string &cert_file, const std::string &key_file)
    {
        ...
        {
            LY_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file("
                                   << cert_file << ") error";
            return false;
        }
        if (SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1)
        {
            LY_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file("
                                   << key_file << ") error";
            return false;
        }
        if (SSL_CTX_check_private_key(m_ctx.get()) != 1)
        {
            LY_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file="
    ...
    SSLSocket::ptr SSLSocket::CreateTCP(lyserver::Address::ptr address)
    {
