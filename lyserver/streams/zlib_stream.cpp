...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    LY_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    LY_ASSERT((window_bits >= 8 && window_bits <= 15));
    LY_ASSERT((memlevel >= 1 && memlevel <= 9));
...
lyserver::ByteArray::ptr ZlibStream::getByteArray() {
